...
/**
 * Unit tests for DeductInventoryUseCase using Mockito.
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("DeductInventoryUseCase Unit Tests")
class DeductInventoryUseCaseTest {

    @Mock
    private InventoryRepository inventoryRepository;

    @InjectMocks
    private DeductInventoryUseCase useCase;

    private DeductInventoryCommand validCommand;
    private Inventory testInventory;

    @BeforeEach
    void setUp() {
        validCommand = new DeductInventoryCommand(100L, 30);
    ...
    @Test
    @DisplayName("Should successfully deduct inventory with valid data")
    void testExecute_Success() {
        ...
        // When
        DeductInventoryResult result = useCase.execute(validCommand);

        // Then
        assertTrue(result.isSuccess());
        assertEquals("Inventory deducted successfully", result.getMessage());
    ...
    void testExecute_DeductAllReserved() {
        // Given
        DeductInventoryCommand command = new DeductInventoryCommand(100L, 50);
        when(inventoryRepository.findByProductId(100L)).thenReturn(Optional.of(testInventory));
        when(inventoryRepository.save(any(Inventory.class))).thenReturn(testInventory);

        // When
        DeductInventoryResult result = useCase.execute(command);
    ...
    @Test
    @DisplayName("Should deduct minimum quantity of 1")
    void testExecute_MinimumQuantity() {
        // Given
        DeductInventoryCommand command = new DeductInventoryCommand(100L, 1);
        when(inventoryRepository.findByProductId(100L)).thenReturn(Optional.of(testInventory));
        when(inventoryRepository.save(any(Inventory.class))).thenReturn(testInventory);
    ...
    void testExecute_NullProductId() {
        // Given
        DeductInventoryCommand command = new DeductInventoryCommand(null, 30);
    ...
    void testExecute_NullQuantity() {
        // Given
        DeductInventoryCommand command = new DeductInventoryCommand(100L, null);
    ...
    void testExecute_ZeroQuantity() {
        // Given
        DeductInventoryCommand command = new DeductInventoryCommand(100L, 0);
    ...
    void testExecute_NegativeQuantity() {
        // Given
        DeductInventoryCommand command = new DeductInventoryCommand(100L, -10);
    ...
    @Test
    @DisplayName("Should fail when insufficient reserved stock")
    void testExecute_InsufficientReservedStock() {
        // Given
        DeductInventoryCommand command = new DeductInventoryCommand(100L, 60);
        when(inventoryRepository.findByProductId(100L)).thenReturn(Optional.of(testInventory));
        ...
        // Then
        assertFalse(result.isSuccess());
        assertTrue(result.getMessage().contains("Insufficient reserved stock"));
        assertTrue(result.getMessage().contains("Reserved: 50"));
        assertTrue(result.getMessage().contains("Requested: 60"));
        assertEquals("INSUFFICIENT_RESERVED_STOCK", result.getErrorCode());
    ...
    void testExecute_RequestOneMoreThanReserved() {
        // Given
        DeductInventoryCommand command = new DeductInventoryCommand(100L, 51);
        when(inventoryRepository.findByProductId(100L)).thenReturn(Optional.of(testInventory));
